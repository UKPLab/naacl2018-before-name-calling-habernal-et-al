# create a distance matrix: for each AH sort all other candidates by distance
# this should be run on as many cores as possible to speed up the computations
...
def compute_parallel(input_file_name: str, output_file_prefix: str):
    ...
    if int(os.sys.argv[1]) not in range(0, 6):
    ...
    for _ in positive_instances_all:
        ...
        positive_instances_by_ids[_.name] = _

    assert len(positive_instances_all) == len(positive_instances_by_ids)

    # split positive instances into chunks (605 each) for 6 jobs
    positive_instances_chunks = chunks(positive_instances_all, 605)
    ...
    for i, chunk in enumerate(positive_instances_chunks):
        for _ in chunk:
            assert isinstance(_, AnnotatedRedditComment)
            if _.name in ids_of_chunks:
                raise Exception("%s from chunk %d already in other chunk!" % (_.name, i))
            ids_of_chunks.add(_.name)

            if 't1_djzvw3b' == _.name:
                print("t1_djzvw3b found in chunk %i" % i)
    ...
    for instance in positive_instances:
        positive_instances_emb_vectors[instance.name] = similarity_helper.average_embeddings_vector(instance.body)
        positive_instances_lengths[instance.name] = len(instance.body)

    for instance in negative_instances:
        negative_instances_emb_vectors[instance.name] = similarity_helper.average_embeddings_vector(instance.body)
        negative_instances_lengths[instance.name] = len(instance.body)
...
if __name__ == "__main__":
    ...
    # now for AH-1 and delta
    compute_parallel("ah-minus1-delta-instances-all.pkl", "ah-minus1-delta-distances")
